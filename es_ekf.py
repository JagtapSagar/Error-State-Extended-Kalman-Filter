...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    C_ns = Quaternion(q_est[k-1,0],q_est[k-1,1],q_est[k-1,2],q_est[k-1,3],None,None).to_mat()
    ...
    # Quaternion estimate
#     quat = Quaternion(euler=imu_w.data[k-1]*delta_t)
#     q_w = quat.w
#     qv = np.array([quat.x,quat.y,quat.z]).reshape(3,1)
#     qv_x = skew_symmetric(qv)
#     row_1 = np.hstack([np.zeros(1).reshape(1,1),-qv.T])
#     row_2 = np.hstack([qv,-qv_x])
#     qv_combined = np.vstack([row_1,row_2])
#     Omega = q_w*np.eye(4) + qv_combined
#     q_est[k] = Omega.dot(q_est[k-1])
    ...
    Q = np.eye(6)
    Q[:3,:3] *= var_imu_f*delta_t**2
    Q[3:,3:] *= var_imu_w*delta_t**2

    # 2. Propagate uncertainty
    # Using error dynamics
